...
class MainActivity : AppCompatActivity(), WiFiDirectDeviceFragment.OnListFragmentInteractionListener {
	...
	val receiveIpAddressServiceConnection = object : ServiceConnection {
		...
		override fun onServiceConnected(componentNameOfServic: ComponentName, service: IBinder) {
			...
			Log.e(LOG_TAG, "onServiceConnected callback in MainActivity called for $componentNameOfServic")
	...
	private val ipAddressBroadcastReceiver: BroadcastReceiver = object : BroadcastReceiver() {
		override fun onReceive(context: Context?, intent: Intent?) {
			...
			Log.e(LOG_TAG, "setting ip address of other device $ipAddressFromOtherDevice in main activity")
	...
	private val newFileReceivedBroadcastReceiver: BroadcastReceiver = object : BroadcastReceiver() {
		override fun onReceive(context: Context?, intent: Intent?) {
			Log.e(LOG_TAG, "onReceive of new file received broadcast receiver")
			...
			Log.e(LOG_TAG, "File was written to $absolutePathToNewlyReceivedFile")
	...
	private fun displayFileInAppropriateNewActivityIfPossible(absolutePathToFile: String) {
		...
		try {
			...
			Log.e(ReceiveFileOrGetIpAddressFromOtherDeviceAsyncTask.LOG_TAG, "No installed app supports viewing this content!", e)
			Snackbar.make(
	...
	private var fineLocationPermissionGranted: Boolean = false
		...
		var locationModeEnabled: Boolean = false
			set(newValueLocationModeEnabled) {
				Log.e(LOG_TAG, "Location mode set to state: $newValueLocationModeEnabled")
	...
	fun notifyWiFiDirectPeerListDiscoveryFinished(discoveredPeers: WifiP2pDeviceList): Unit {
		Log.e(LOG_TAG, "Discovered WiFi Direct peers:\n" +
		...
		if (wiFiDirectPeers.isEmpty()) {
			Log.e(LOG_TAG, "No WiFi Direct devices found in current scan")
		}
	}
	...
	override fun onListFragmentInteraction(wiFiDirectDevice: WifiP2pDevice): Unit {
		wiFiDirectDevice.let { clickedDeviceItem ->
			Log.e(LOG_TAG, "$clickedDeviceItem\n has been clicked")
		...
		wiFiDirectChannel.also {
			...
			wiFiDirectManager?.connect(wiFiDirectChannel, wiFiDirectConnectionConfig, object : WifiP2pManager.ActionListener {
				...
				override fun onSuccess(): Unit {
					Log.e(LOG_TAG, "Initiation of connection to $wiFiDirectDeviceToConnectTo succeeded")
				...
				override fun onFailure(reason: Int): Unit {
					...
					Log.e(LOG_TAG, "Initiation of connection to $wiFiDirectDeviceToConnectTo failed: $failureReason")
	...
	private fun requestFineLocationPermissionForWiFiDirect(): Unit {
		...
		if (ActivityCompat.shouldShowRequestPermissionRationale(
			...
			Log.e(LOG_TAG, "Fine Location permission was denied in the past! TODO: show an explanation before retrying")
		...
		when (requestCode) {
			ACCESS_FINE_LOCATION_PERMISSION_REQUEST_CODE -> {
				...
				if (grantResults.getOrNull(0) == PackageManager.PERMISSION_GRANTED) {
					...
					Log.e(LOG_TAG, "Fine Location permission denied, cannot use WiFi Direct! TODO: disable all functionality dependent on that")
	...
	private fun discoverWiFiDirectPeers(): Unit {
		...
		wiFiDirectManager?.discoverPeers(wiFiDirectChannel, object : WifiP2pManager.ActionListener {
			...
			override fun onSuccess(): Unit {
				Log.e(LOG_TAG, "Initiating peer discovery successful")
			...
			override fun onFailure(reasonCode: Int): Unit {
				...
				Log.e(LOG_TAG, "Initiating peer discovery failed: $failureReason")
	...
	override fun onCreate(savedInstanceState: Bundle?): Unit {
		Log.e(LOG_TAG, "onCreate starting...")
		super.onCreate(savedInstanceState)
		setContentView(R.layout.activity_main)
		initializeWiFiDirect()

		Log.e(LOG_TAG, "registering local broadcast receiver for receiving ip address and newly received file of other device from service...")
	...
	override fun onStart() {
		...
		Log.e(LOG_TAG, "Location mode queried onResume, state: $currentLocationModeState")
		locationModeEnabled = currentLocationModeState

		Log.e(LOG_TAG, "registering location mode changed broadcast receiver... : $locationModeChangedBroadcastReceiver")
		registerReceiver(locationModeChangedBroadcastReceiver, locationModeChangedIntentFilter)


		Log.e(LOG_TAG, "registering wifi direct broadcast receiver... : $wiFiDirectBroadcastReceiver")
		...
		if (haveFineLocationPermission) {
			Log.e(LOG_TAG, "We already have Fine Location permission")
			fineLocationPermissionGranted = true
		} else {
			fineLocationPermissionGranted = false
			Log.e(LOG_TAG, "We do not have Fine Location permission, requesting...")
		...
		Log.e(LOG_TAG, "Starting new receive ip address and file service, still testing...")
		...
		Intent(this, WiFiDirectBackgroundService::class.java).also { intent ->
			Log.e(LOG_TAG, "Trying to bindService to ${WiFiDirectBackgroundService::class.simpleName}...")
	...
	override fun onResume(): Unit {
		Log.e(LOG_TAG, "onResume starting...")
		super.onResume()
	}
	...
	override fun onPause(): Unit {
		Log.e(LOG_TAG, "onPause starting...")
		super.onPause()
	}

	override fun onStop() {
		Log.e(LOG_TAG, "onStop starting...")

		Log.e(LOG_TAG, "unregistering Location Mode changed Broadcast Receiver... : $locationModeChangedBroadcastReceiver")
		unregisterReceiver(locationModeChangedBroadcastReceiver)

		if (wiFiDirectBroadcastReceiver != null) {
			Log.e(LOG_TAG, "unregistering wifi direct broadcast receiver... : $wiFiDirectBroadcastReceiver")
			unregisterReceiver(wiFiDirectBroadcastReceiver)
		} else {
			Log.e(LOG_TAG, "Did not unregister WiFi Direct Broadcast Receiver in onPause because it was null!")
		}
	...
	override fun onDestroy() {
		Log.e(LOG_TAG, "unregistering local broadcast receiver for receiving ip address or new file of other device from service...")
	...
	fun onClickedOpenFileButton(view: View): Unit {
		...
		Log.e(LOG_TAG, "open file button clicked")
	...
	private fun disconnectWiFiDirectConnection(): Unit {
		if (wiFiDirectManager == null || wiFiDirectChannel == null) {
			Log.e(LOG_TAG, "Could not request disconnect because WiFi Direct Manager or Channel are null")
		...
		wiFiDirectManager?.requestGroupInfo(wiFiDirectChannel) { wifiDirectGroup: WifiP2pGroup? ->
			Log.e(LOG_TAG, "Current WiFi Direct Group: $wifiDirectGroup")
			if (wifiDirectGroup == null) {
				...
				Log.e(LOG_TAG, "Could not request disconnect because current WiFi Direct Group is null")
			...
			if (wiFiDirectManager == null || wiFiDirectChannel == null) {
				...
				wiFiDirectManager?.removeGroup(wiFiDirectChannel, object : WifiP2pManager.ActionListener {
					override fun onSuccess() {
						Log.e(LOG_TAG, "Successfully disconnected from group: $wifiDirectGroup")
					...
					override fun onFailure(reason: Int) {
						val failureReason: String = getFailureReasonForWiFiDirectActionListener(reason)
						Log.e(LOG_TAG, "Error when trying to disconnect from $wifiDirectGroup, failed because: $failureReason")
	...
	override fun onActivityResult(requestCode: Int, resultCode: Int, resultIntentWithData: Intent?): Unit {
		...
		if (requestCode == OPEN_FILE_WITH_FILE_CHOOSER_REQUEST_CODE && resultCode == Activity.RESULT_OK) {
			...
			val uriOfSelectedFile: Uri = resultIntentWithData?.data?.also { uri ->
				Log.e(LOG_TAG, "Uri of file to send, chosen by user: $uri")
				dumpContentUriMetaData(uri)
			} ?: TODO("handle error, did not receive uri of content in intent data from the system file chooser")


			Log.e(LOG_TAG, "Currently connected WiFi Direct Device Info: $wiFiDirectGroupInfo")
	...
	fun dumpContentUriMetaData(uriWithContentScheme: Uri): Unit {
		...
		if (uriWithContentScheme.scheme != "content") {
			Log.e(LOG_TAG, "Cannot currently dump metadata of uris that do not have the content uri scheme!")
			return
		}
		...
		cursor?.use {
			...
			if (it.moveToFirst()) {
				...
				Log.e(LOG_TAG, "Display Name: $displayName")
				...
				Log.e(LOG_TAG, "File size: $fileSize bytes")
				// everything went well, no uncaught errors, return without logging an error
				return
			}
		}
		Log.e(LOG_TAG, "couldn't get metadata of file with uri $uriWithContentScheme")
	}
}