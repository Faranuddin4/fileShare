					...
					if (networkInfo?.isConnected == true) {
						...
						val connectionInfoListener = WifiP2pManager.ConnectionInfoListener { wiFiDirectInfo ->
							...
							if (wiFiDirectInfo.groupFormed) {
								...
								// start server on both group owner and client, because that is
									// the way the accept incoming connections with data, but
								// the client cannot be contacted until the group owner knows its
									// ip address, which we will tell them by connecting on
								// another por
